...
class Booking extends Component {
  ...
  handleBookingClick = async (id, { checkin, checkout}) => {
    const isValidBooking = this.validateBooking(checkin, checkout) 

    if(isValidBooking) {
      const isOccupied = await this.confirmBooking(id, checkin, checkout)
      console.log(isOccupied.isOccupied)
      isOccupied.isOccupied ? alert('Unsuccessful Booking') : alert('successful Booking')
    }
  }
  ...
  confirmBooking = async (id, checkin, checkout) => {
    try {
      return await fetch(`/booking/${id}/?checkin=${checkin}&checkout=${checkout}`, {
        method: 'GET'
      }).then(data => data.json())
    } catch(err) {
